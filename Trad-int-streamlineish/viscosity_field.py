...
if __name__ == "__main__": # view the viscosity map if the script is run directly
    ...
    x = linspace(0,params["L_x"],int(np.ceil(10*params["L_x"])))
    y = linspace(-params["L_y"]/2.,params["L_y"]/2.,int(np.ceil(10*params["L_y"])))

    X, Y = meshgrid(x,y)

    #Border = np.asarray(void_borders(ind_func))
