from numpy import random, zeros, meshgrid, linspace, concatenate, array, exp, average, arange
...
def run_simulation(params, Visc):

    integrable_calcualte_forces = lambda t,x: calculate_forces(t,x,params,Visc) # this function only takes t and x so it can be used by an integrator
    ...
    if params["Until_end"]: # the point of this section is to estimate how long we need to run the integrator for in order to get an electron to the other node
        ...
        a = average(params["alpha"]) * params["V"]/ (0.5 * params["L_x"]) # electric field acceleration
    ...
    # ------------------------
    xo = random.random(n) * 0.1 # start with a very small amount of variation just so that the curvature is non-zero and thus the force at each region is not set to zero.
    yo = params['y_pos']
    ...
    particles, = ax1.plot(initial[0:n], initial[2*n : 3*n], markersize=2,marker='none', ls='-', color='b') # this is the artist to be updated
    ...
    with writer.saving(fig, f"dendrite_growth_simulation"+'.mp4', dpi=50): # this enables streaming the simulation data to a file concurrent with the simualtion - enables an approximate 5x speed up
